...
import { ArrowRight, CalendarDays, ShoppingCart, Sparkles, UtensilsCrossed } from "lucide-react";

const trustPills = ["Digital Menu", "Table Booking", "Online Ordering", "Loyalty Tools"];

const statCards = [
  { value: "24/7", label: "Ordering access", className: "top-8 left-0 lg:left-10" },
  { value: "150+", label: "Dishes listed", className: "top-28 right-0 lg:right-8" },
  { value: "97%", label: "Menu clarity", className: "bottom-28 left-0" },
  { value: "12+", label: "Events managed", className: "bottom-8 right-10" },
];
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 items-center gap-14 px-6 lg:grid-cols-2 lg:gap-20">
        <div>
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }} className="mt-8 max-w-2xl text-lg leading-relaxed text-gray-600">
            Restaurant websites that showcase menus, support online ordering, and help food businesses convert more diners with a premium digital presence.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.05 }} className="mt-12 flex flex-col items-stretch gap-4 sm:flex-row sm:items-center sm:gap-5">
        ...
        <div className="relative hidden h-[700px] items-center justify-center lg:flex">
          ...
          <motion.div animate={{ y: [0, -15, 0] }} transition={{ duration: 6, repeat: Infinity }} className="relative h-[540px] w-[420px] overflow-hidden rounded-[40px] border border-white/70 bg-white/80 shadow-[0_30px_120px_rgba(15,23,42,0.12)] backdrop-blur-3xl">
            ...
            <img src="https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1200&auto=format&fit=crop" alt="Restaurant" className="absolute inset-0 h-full w-full object-cover opacity-85" />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-8">
              ...
              <h3 className="mt-5 text-3xl font-black text-white">Modern Restaurant Experience</h3>
              <p className="mt-4 leading-relaxed text-slate-200">
                Menus, booking flows, and ordering experiences designed to increase guest conversion.
              </p>
            </div>
          </motion.div>

          {statCards.map((card, index) => (
            <motion.div key={card.label} animate={{ y: [0, -10, 0] }} transition={{ duration: 4 + index, repeat: Infinity }} className={`absolute hidden lg:block ${card.className}`}>
